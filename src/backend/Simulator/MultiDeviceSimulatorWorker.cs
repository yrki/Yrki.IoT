...
public class MultiDeviceSimulatorWorker(IBus bus, ILogger<MultiDeviceSimulatorWorker> logger) : BackgroundService
{
    ...
    private const string SimulatedGatewayId = "sim-gateway-01";

    private static readonly SimulatedDevice[] Devices =
    [
        new("67000100", 20.0, 25.0, 35.0, 65.0, 400, 1600, 30, 55, 0.5, 1.0, 0.0, 1.5, BaseRssi: -45),
        new("67000101", 18.0, 22.0, 40.0, 70.0, 450, 1100, 25, 40, 0.3, 0.8, 0.2, 1.0, BaseRssi: -58),
        new("67000102", 21.0, 26.0, 30.0, 58.0, 550, 1300, 28, 48, 0.1, 1.3, 0.6, 0.9, BaseRssi: -72),
        new("67000103", 19.0, 24.0, 42.0, 68.0, 500, 1200, 32, 50, 0.7, 1.1, 0.4, 1.7, BaseRssi: -88),
        new("67000104", 17.5, 23.5, 38.0, 62.0, 420, 980, 27, 37, 0.9, 0.4, 0.8, 0.5, BaseRssi: -100),
    ];
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        logger.LogInformation(
            "Multi-device simulator started — {Count} devices, {Interval}s interval, ~{Period} min cycle, gateway {GatewayId}",
            Devices.Length, IntervalSeconds, CycleSteps * IntervalSeconds / 60, SimulatedGatewayId);
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            foreach (var device in Devices)
            {
                ...
                var rssi = device.BaseRssi + _random.Next(-10, 11);
                ...
                logger.LogInformation(
                    "Published WMBus payload for {AddressHex} via {GatewayId} (RSSI {Rssi})",
                    device.AddressHex, SimulatedGatewayId, rssi);
            }
